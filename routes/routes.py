...
@router.get(
    "/rank/{metric}",
    response_description="retrieving llm rankings based on average performance",
)
async def rank_models(metric: str):
    ...
    if metric not in valid_metrics:
        return ErrorResponseModel(
            "invalid metric type",
            400,
            f"Invalid metric. Valid metrics are: {', '.join(valid_metrics)}",
        )
    ...
    return ResponseModel(
        {"metric": metric, "ranking": ranked_models},
        f"LLM rankings for {metric} retrieved ",
    )


@router.get(
    "/get-metrics/{llm}",
    response_description="Retrieving simulation metrics from the database",
)
